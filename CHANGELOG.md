...
- Other
  ...
  - Releases are now published automatically when a version tag is pushed, with the release notes taken from this changelog. Releases had been cut by hand and had fallen three versions behind, which matters because the in-script update check reads the latest release — so it kept reporting that an older version was current. The released file is now named for its version, `4chan-XTd-<version>.user.js`.
...
- Bugfixes
  - Fixed a critical crash during `Header` initialization caused by invalid `this` context binding, which halted the execution of the entire userscript.
  - Fixed Quick Reply initialization crash occurring when persistent QR was disabled.
  - Restored draggable functionality and layout positioning across all UI dialog windows (including the QR Window and Settings Panel).
  - Resolved missing Captcha bridge module reference introduced during the migration.
