...
var Keybinds: any = {
  ...
  keydown(e) {
    ...
    if (key === Conf['Close']) {
      ...
      } else if ((Embedding as any).lastEmbed) {
    ...
    if (key === Conf['Toggle threading'] && (QuoteThreading as any).ready) {
    ...
    // Images
    if (key === Conf['Expand image'] && (ImageExpand as any).enabled && threadRoot) {
    ...
    if (key === Conf['Expand images'] && (ImageExpand as any).enabled) {
